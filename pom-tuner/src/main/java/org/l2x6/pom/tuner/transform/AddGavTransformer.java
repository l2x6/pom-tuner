...
/**
 * A generic adder of {@code pom.xml} GAV elements such as {@code <plugin>} or {@code <extension>}.
 *
 * @author <a href="https://github.com/ppalaga">Peter Palaga</a>
 * @since  5.0.0
 */
public class AddGavTransformer<P extends ContainerElement, T extends GavtcsElement, THIS extends AddGavTransformer<P, T, THIS>>
        extends AbstractAddTransformer<P, T, Gav, THIS> {
    ...
    public THIS intoProfile(Function<TransformationContext, ProfileElement> profileSelector) {
        return (THIS) new AddGavTransformer<>(
                profileSelector,
                profileToParentElement,
                createChild,
                comparator,
    ...
    public THIS intoProfile(String profileId) {
        return (THIS) new AddGavTransformer<P, T, THIS>(
     ...
     */
    @SuppressWarnings("unchecked")
    public THIS at(Comparator<Gav> position) {
        return (THIS) new AddGavTransformer<P, T, THIS>(
                profileSelector,
                profileToParentElement,
                createChild,
    ...
    /**
     * @param  gav the {@link Gav} after which the new element should be added; if there is no such
     *             element, the new element is added at the last position
     * @return     a copy of this {@link AddGavTransformer} instance with the
     *             {@link #comparator} adjusted
     * @since      5.0.0
     */
    public AddGavTransformer<P, T, THIS> after(Gav gav) {
        return new AddGavTransformer<P, T, THIS>(
                profileSelector,
                profileToParentElement,
                createChild,
                Comparators.after(gav),
                postprocessors);
    }

    /**
     * @param  gav the {@link Gav} before which the new element should be added; if there is no such
     *             element, the new element is added at the last position
     * @return     a copy of this {@link AddGavTransformer} instance with the
     *             {@link #comparator} adjusted
     * @since      5.0.0
     */
    public AddGavTransformer<P, T, THIS> before(Gav gav) {
        return new AddGavTransformer<P, T, THIS>(
                profileSelector,
                profileToParentElement,
                createChild,
                Comparators.before(gav),
                postprocessors);
    }
